...
void directions(SDL_Rect *body, int *dir, int *tpCount, TurningPoint *turningPoints){
	if (body[0].x < body[0].h/2) { // if snake head meets left border
		if (dir[0] == LEFT && body[0].y > 36+body[0].h/2) {
	...
	if (body[0].x == SCREEN_WIDTH - body[0].w) {
		if (dir[0] == RIGHT && body[0].y < SCREEN_HEIGHT - body[0].h){
	...
	if (body[0].y < 36+body[0].h/2) {
		// printf("%d", headRect.x);
		if (dir[0]== UP && body[0].x == SCREEN_WIDTH-body[0].w){
			dir[0] = LEFT;	
	...
	if (body[0].y > SCREEN_HEIGHT-1.5*body[0].h) {
		if (dir[0] == DOWN && body[0].x == SCREEN_WIDTH-body[0].w){
			dir[0] = LEFT;
			turningPoints[(*tpCount)++] = (TurningPoint){body[0].x, body[0].y, LEFT};
		}
		if (dir[0]== DOWN && body[0].x < SCREEN_WIDTH-body[0].w){
...
int main(){
	...
	SDL_Rect Fruit = {fruitX, fruitY, 10, 10}; 
	...
	while(!quit) {
		...
		// if(headRect.x == SCREEN_WIDTH-1 && dir == RIGHT && headRect.y != 35 && headRect.y != SCREEN_HEIGHT){
		// 	dir = RIGHT;
		// }
            // moving the head
		int selfCollision = 0;
